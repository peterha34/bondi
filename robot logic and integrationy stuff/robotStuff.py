...
def server_start(pi_address):
    ...
    if initialised:
		...
		Thread(target=socket_broker, args=(pi_address, vision_queue, conveyor_queue, treatment_queue, load_robot_queue, stash_robot_queue)).start()
		...
		Thread(target=stacker_State_Machine, args=(context, stack_robot_queue, loader_robot_queue, conveyor_queue)).start()
