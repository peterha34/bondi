...
def main():
    ...
    while True:
        if SENSOR_TEMP:
            temp_val = tag.IRtemperature.read()
            print('Temp: {}'.format(temp_val))
        if SENSOR_HUM:
            hum_val = tag.humidity.read()
            print('Humidity: {}'.format(hum_val))
        if SENSOR_BAR:
            bar_val = tag.barometer.read()
            print('Humidity: {}'.format(bar_val))
        if SENSOR_ACCE:
            acce_val = tag.accelerometer.read()
            print('Accelerometer: {}'.format(acce_val))
        if SENSOR_MAG:
            mag_val = tag.magnetometer.read()
            print('Magetometer: {}'.format(mag_val))
        if SENSOR_GYRO:
            gyro_val = tag.gyroscope.read()
            print('Gyroscope: {}'.format(gyro_val))
        if SENSOR_LIGHT:
            tag.lightmeter.enable()
            light_val = tag.lightmeter.read()
            print('Light: {}'.format(light_val))
        if SENSOR_BATT:
            tag.battery.enable()
            batt_val = tag.battery.read()
            print('Battery: {}'.format(batt_val))

        tag.waitForNotifications(POLL_TIME)
        
    tag.disconnect()
    del tag
    
""" Getting sensor value.. I tried something here but its fked so imma give up"""
##def SensorVal(self):
##    val = {}
##    tag = SensorTag(DEVICE)
