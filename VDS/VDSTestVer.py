...
def isColour(HSVvalue, colour):
    ...
    if HSVvalue[0]>=colBounds[colLow].R and HSVvalue[0]<=colBounds[colHigh].R and\
    HSVvalue[1]>=colBounds[colLow].G and HSVvalue[1]<=colBounds[colHigh].G and\
    HSVvalue[2]>=colBounds[colLow].B and HSVvalue[2]<=colBounds[colHigh].B:
        return True;
    return False;
